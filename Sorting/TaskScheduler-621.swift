...
//
/*
 https://leetcode.com/problems/task-scheduler/description/?envType=company&envId=amazon&favoriteSlug=amazon-three-months
...
func leastInterval2(_ tasks: [Character], _ n: Int) -> Int {
        ...
        maxCounter = dict.values.filter({ $0 == maxFreq}).count
        return max(tasks.count, maxCounter + (maxFreq - 1) * (n + 1))
}
